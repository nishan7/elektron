...
function RecentAlerts() {
  ...
  const formatTimestamp = useCallback((timestamp) => {
    // Format to Pacific Time (Los Angeles) using a representative timeZone, DATE ONLY
    try {
      // Use toLocaleDateString to get only the date part
      return new Date(timestamp).toLocaleDateString('en-US', { 
        timeZone: 'America/Los_Angeles', // Correct timezone for PDT/PST
        year: 'numeric', 
      ...
      return new Date(timestamp).toISOString().split('T')[0]; // Fallback to ISO date string
    }
  }, []);
            ...
            <Typography variant="caption" color="textSecondary">
              ...
              Last updated: {(() => {
                try {
                  ...
                  console.error("Error formatting lastUpdateTime:", e);
                  return lastUpdateTime.toISOString(); // Fallback
                }
              })()}
            </Typography>
          </Box>
